...
public class Chess {
   ...
   // private MenuIF menu;
     ...
     * @UI_FactoryIF uiFactory Inject the UI factory responsible for building UI components CLI or GUI or other.
     */
    public Chess(UI_FactoryIF uiFactory) {
    ...
    public void mainMenu(){
       ...
       UI_Interface<Integer> options_menu =  ui_factory.BuildMainOptions();
       
        // looping until the user selects 3 to quit
        while(option != this.options.size()-1){
        
        	
        	//Observer for selection on the options menu by the user
        	options_menu.setFinishedObserver(new UIFinishedObserverIF<Integer>() {
          		 /**
          		  ...
          		  */
				@Override
				public void inputFinished(Integer result,UI_Interface<Integer> sourceUI) {
					doMenuOptions(result);
				}
        	});
        	options_menu.showUI();
     ...
     */
    private void doMenuOptions(int option) {
    	if (option == 0){
    		player1LogIn();
        // player chose to create an account
        }else if(option == 1){
        	guestPlay();
        }else if (option == 2){
            createAccount();
        // player chose to change username
        } else if (option == 3 ){
        	changeUserID();
        } else if (option == 4 ){
        	colorOptions();
    ...
    public void player1LogIn() {
    	  String prompt1 = "Log in Player 1";
    	  UI_Interface<PlayerIF> player1Login =ui_factory.buildLogInScreen(playerDataBase,prompt1);
    	  //Observe for when the user has interacted with the UI
    	  player1Login.setFinishedObserver(new UIFinishedObserverIF<PlayerIF>() {
	   		  ...
	   		  */
			@Override
			public void inputFinished(PlayerIF result,UI_Interface<PlayerIF> sourceUI) {
				//Chess.this means this of outer class (as this is an inner class technically)
				Chess.this.playerWhite = result;	
				player2LogIn();//success log in next player
			}
    ...
    public void player2LogIn() {
      String prompt2 = "Log in Player 1";
   	  UI_Interface<PlayerIF> player2Login =ui_factory.buildLogInScreen(playerDataBase,prompt2);
   	  //Observer for when the user has interacted with the UI
   	  player2Login.setFinishedObserver(new UIFinishedObserverIF<PlayerIF>() {
			...
			public void inputFinished(PlayerIF result,UI_Interface<PlayerIF>sourceUI) {
				//Chess.this means this of outer class (as this is an inner class technically)
				Chess.this.playerBlack = result;	
				
				//Play a game if both players defined then play game
				if(Chess.this.playerBlack != null && Chess.this.playerWhite != null)
		              Chess.this.newGame();
    ...
    private void createAccount() {
    	...
    	createAccount.setFinishedObserver(new UIFinishedObserverIF<PlayerIF>() {
      		 /**
      		  * Once the user has finished interacting with the UI
      		  * @param newPlayer The value decided by the user
      		  * @param sourceUI The source UI that invoked this method
      		  */
			@Override
			public void inputFinished(PlayerIF newPlayer,UI_Interface<PlayerIF> sourceUI) {
				...
				//an inner class.
				Chess.this.playerDataBase.put(newPlayer.getID(), newPlayer);			
			}
    ...
    public void colorOptions() {
    	 ...
    	 colorOptions.setFinishedObserver(new UIFinishedObserverIF<Boolean>() {
			...
			public void inputFinished(Boolean result,UI_Interface<Boolean> sourceUI) {
			    if (result)
			    	Chess.this.boardColor ="Color";
			    else
			    	Chess.this.boardColor = "Mono";
			}
    ...
    public void move(File fromF, Rank fromR, File toF, Rank toR) {
        ...
        if (initial.validateMove(new Position(fromR, fromF), new Position(toR, toF))) {
        	...
        	cli_message.showUI();
           // this.menu.showError("Invalid Move!");
        }
    }

    /**
     * Handles loging in the player until the player decides they dont
     * want to log in or they log in successfully
     * 
     * @return The player object that logged in
     */
    /*
    public PlayerIF loginPlayer(){
        PlayerIF player = null;
        boolean tryAgain = true;

        do{
            player = menu.login(this.playerDataBase);
            // if null, login was invalid
            if(player == null){
                menu.showError("Invalid UserID or Password");
                tryAgain = menu.getConfirmation("Try Again: ");
            }else{
                tryAgain = false;
            };
        }while(tryAgain); // loop until user enters a valid login
    ...
    public boolean playerChooseFrom(PlayerIF playerTurn ) {
        ...
        posInput.setFinishedObserver(new UIFinishedObserverIF<String>() {
			...
			public void inputFinished(String result,UI_Interface<String>sourceUI) {
				...
				SquareIF[][] squares = Chess.this.board.getSquares();//Squares from board
		        ...
		        if(fromPosString.equals("q")){
		        	...
		        	Position from = Chess.this.parsePosition(fromPosString);
		        	/////////////////////////////////
		        	//If move is invalid
		        	////////////////////////////////
		        	...
		        	//////////////////////////////////
		        		...
		        		Chess.this.currentFrom = from;//GLOBALIZE
			...
			public void inputFinished(String toPosString,UI_Interface<String>sourceUI) {
				 ...
				 if(toPosString.equals("q")){	
					...
					//they will be repromted
                     Chess.this.currentFrom = null;
                     Chess.this.currentTo = null;
                 }else {
                	Position to = Chess.this.parsePosition(toPosString);
                	...
                	if(to == null) {
                		...
                		Chess.this.currentTo = to;//GLOBALIZE
                	}
                 }
     ...
     */
    private boolean checkStatus(boolean whiteTurn) {
        if(isDraw(whiteTurn)){
     ...
     */
    private boolean isDraw(boolean whiteTurn) {
     ...
     * @return true if this has been met, false otherwise
     */
    private boolean isStalemate(boolean whiteTurn){
        Position kingPos;
     ...
     */
    private Position getKingPosition(GameColor color){
     ...
     * white's turn.
     */
    private void setCheckAndCheckmate(boolean whiteTurn) {
     ...
     */
    private boolean isCheck(Position king, GameColor color) {
        PieceIF piece;
        for (Rank r : Rank.values()) {
            for (File f : File.values()) {
     ...
     */
    private boolean isCheckmate(Position king, GameColor color) {
        PieceIF piece;
        for (Rank r : Rank.values()) {
            for (File f : File.values()) {
                piece = board.getPiece(r, f);
     ...
     */
    private boolean checkMoveThenUndo(Position from, Position to, Position king, GameColor color) {
        boolean check;
